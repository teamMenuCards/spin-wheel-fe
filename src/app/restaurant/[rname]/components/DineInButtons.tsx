  ...
  <div className="flex flex-col items-center w-full max-w-[400px] mt-8 mx-auto px-4 relative">
    ...
    {options?.map((item, index) => (
      <NextLink key={index} href={item.path} passHref>
        <div
          className="flex items-center px-6 py-4 bg-white rounded-lg mb-4 shadow-md w-full max-w-[400px] mx-auto cursor-pointer relative border border-gray-200 backdrop-blur-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:bg-gray-50"
        >
          {/* Icon */}
          {item.value === "Menu" && (
            <Image
