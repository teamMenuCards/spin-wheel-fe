...
const getReviewLinks = () => {
	...
	const handleSubmit = (_selectedOptions: string, _otherText?: string) => {
		setShowThankYouPopup(true)
		setShowPopup(false)
	}
	...
	const getDiscountValue = () => {
		...
		if (
			currentSegment.discountType === "percentage" &&
			currentSegment.discountValue
		) {
		...
		if (
			currentSegment.discountType === "fixed" &&
			currentSegment.discountValue
		) {
			return currentSegment.discountValue
		}
			...
			<div className="fixed inset-0 z-50 flex items-center justify-center bg-white">
				<div className="bg-white rounded-3xl p-8 text-center shadow-2xl border border-gray-200 relative overflow-hidden">
					...
					<div className="relative z-10">
						...
						{restaurantId && !spinnerData && (
							<p className="text-yellow-500 text-sm mt-2">
								No spinner data found for this restaurant
							</p>
						)}
					</div>
				</div>
			...
			{!spinFinished && !showPopup && (
				<div className="fixed inset-0 z-50 flex flex-col bg-white h-screen items-center justify-between py-4 sm:py-6 lg:py-8 px-4 sm:px-6">
					...
					<div className="absolute inset-0 overflow-hidden pointer-events-none">
						...
						<div
							className="absolute bottom-8 sm:bottom-12 right-8 sm:right-12 w-32 sm:w-40 lg:w-44 h-32 sm:h-40 lg:h-44 bg-gradient-to-r from-pink-100 to-indigo-100 rounded-full blur-3xl opacity-60 animate-pulse"
							style={{ animationDelay: "1s" }}
						></div>
						<div
							className="absolute top-1/2 left-1/4 w-20 sm:w-24 lg:w-28 h-20 sm:h-24 lg:h-28 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-full blur-2xl opacity-50 animate-pulse"
					...
					<div className="relative z-10 h-12 sm:h-16 lg:h-20 flex items-center justify-center"></div>
				</div>
			)}
