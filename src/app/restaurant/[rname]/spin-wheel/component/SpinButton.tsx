...
export const SpinButton: React.FC<SpinButtonProps> = ({
    ...
    const handleClick = () => {
        ...
        // 🔥 Lock spinning immediately when user clicks
    ...
    // Reset isClicked when spinner stops
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col items-center space-y-3 mt-6">
            <button
                className={`relative overflow-hidden font-black py-5 px-12 rounded-3xl transition-all duration-200 group text-lg min-w-[200px] touch-manipulation ${!canSpin || closeModal || isSpinning
                ...
                onMouseDown={(e) => {
                    ...
                    e.currentTarget.style.background =
                        "linear-gradient(to right, rgb(147, 51, 234), rgb(236, 72, 153), rgb(220, 38, 38))"
                }}
                onMouseUp={(e) => {
                    if (!canSpin || closeModal || isSpinning) return
                    e.currentTarget.style.background =
            ...
            >
                <span className="relative z-10 flex items-center justify-center gap-3">
                    <span className="tracking-wider text-lg font-black transform group-active:scale-95 transition-transform duration-150">
                        SPIN TO WIN
                    </span>
                    <svg
                        className={`w-6 h-6 transform transition-all duration-300 ${isSpinning || isClicked
                                ? "animate-spin"
